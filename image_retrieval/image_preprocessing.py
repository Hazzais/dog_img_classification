...
image_path_processed = r'D:\Documents\PythonDoc\Photo_classification\Images\new_test\Processed'
image_path_caltech256 = r'D:\Documents\PythonDoc\Photo_classification\Images\new_test\Caltech256\256_ObjectCategories'
...
def process_images(image_list, folder, x_pixels=64, y_pixels=64,
                   test_num=None, print_status=False):
...
def show_picture(image_array, image_labels=None, number=0, cmap='seismic',
                 x_pixels=64, y_pixels=64):
...
def caltech256_images(folder_base, test_num=None):
    ...
    for i, folder in enumerate(subfolders):
        ...
        print('For folder ' + str(i+1) + ' of ' + str(len(subfolders)) +
              ': --- ' + image_label)
        base_images = retrieve_images(folder)
        base_images_processed = process_images(base_images,
                                               folder,
                                               x_pixels=x_pixels,
                                               y_pixels=y_pixels)
...
hf_images_processed = process_images(hf_images,
                                     image_path_raw,
                                     x_pixels=x_pixels,
                                     y_pixels=y_pixels)

all_ct256_labels, all_ct256_images = caltech256_images(image_path_caltech256,
                                                       test_num=None)
...
show_picture(all_ct256_images,
             image_labels=all_ct256_labels,
             number=3460,
             cmap='gray')
show_picture(all_images_norm,
             image_labels=all_ct256_labels,
             number=0,
             cmap='gray')
