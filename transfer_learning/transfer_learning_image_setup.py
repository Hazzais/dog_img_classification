...
# Input paths
img_path_dogs = r'D:\Documents\PythonDoc\Photo_classification\Images\StanfordDogs'
...
def send_images_to_staging(in_folder, out_folder, dataset_dog=False, n='all'):
    # Extract images from Stanford Dogs and Caltech datasets and save to an
    # intermediate folder
    ...
    for i, f in enumerate(subfolders):
        ...
        print("Performing subfolder " + str(i + 1) + " of " + str(
            len(subfolders)))
        ...
        append_to_fname = '[' + (
            'dog' if is_dog else 'nodog') + '](' + image_label + ')'
...
def move_staging_to_train_test(img_dict, staging_folder, output_folder,
                               copy=True):
    ...
    for img_nm, img_lbl in img_dict.items():
        ...
        if img_lbl == 1:
            ...
            output_final_filepath = os.path.join(output_folder, 'nodog',
                                                 img_nm)
...
# Images are copied to staging area before being assigned into training and
# testing sets
send_images_to_staging(img_path_caltech, img_path_staging, dataset_dog=False,
                       n='all')
send_images_to_staging(img_path_dogs, img_path_staging, dataset_dog=True,
                       n='all')
...
# Get a training set
X_img_train, tmp_X_valid_test, y_img_train, tmp_y_valid_test = train_test_split(
    all_staged,
    class_label,
    test_size=0.3,
    random_state=12)
...
X_img_valid, X_img_test, y_img_valid, y_img_test = train_test_split(
    tmp_X_valid_test,
    tmp_y_valid_test,
    test_size=0.5,
    random_state=12)
