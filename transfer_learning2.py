...
"""
- add dropout layers
- add validation data into fit model
...
#for i, layer in enumerate(model_use.layers):
#    print(i,layer.name)

# Make ResNet model layers not trainable
...
# Training data
data_train = datagen_train.flow_from_directory(os.path.join(img_path,'train'),
...
data_test_infit = datagen_test.flow_from_directory(os.path.join(img_path,'test'),
                                               target_size=(x_pixels, y_pixels),
                                               batch_size=32,
                                               class_mode='binary',
                                               color_mode='rgb',
...
# Validation data - for use after model fiited. Use batch size of one and no
# shuffle as this allows easy mapping back to filenames.
data_test = datagen_test.flow_from_directory(os.path.join(img_path,'test'),
                                               target_size=(x_pixels, y_pixels),
                                               batch_size=1,
                                               class_mode='binary',
                                               color_mode='rgb',
...
# Number of steps needed per epoch is the number needed for each image to go
# through, batch_size at a time
step_size_train = data_train.n//data_train.batch_size

# Train the model - the long bit. Fit on training data generator and provide
...
model_use.save(os.path.join(r'D:\Documents\PythonDoc\Photo_classification\models', model_name + '.h5'))
...
model_use = load_model(os.path.join(r'D:\Documents\PythonDoc\Photo_classification\models', model_name + '.h5'))
...
pred_prob = model_use.predict_generator(data_test, steps=total_test_images, verbose=1)
...
def rand_check(index=None):
    ...
    if index==None:
        ...
        raise ValueError("There are only " + str(len(pred_prob_rev)) + "images to display")
    elif index<0:
    ...
    ((check_image_prob>=0.5) and (check_image_true==1)) or ((check_image_prob<0.5) and (check_image_true==0)) else '#ea1212'
...
def show_results(tp_ind, tn_ind, fp_ind, fn_ind, n_tp=5, n_tn=5, n_fp=5, n_fn=5, n_cols=5):
    ...
    def add_type(inds, ns, string):
        for x in range(min(len(inds),ns)):
            ...
            all_imgs.append(image.load_img(os.path.join(img_path, 'test', img_name)))
            all_probs.append(pred_prob_rev[x_this_ind])
            all_title.append(string + ": {0:.1%}".format(pred_prob_rev[x_this_ind][0]))
    ...
    fig, ax = plt.subplots(n_rows, n_cols,figsize=(10,8), sharex=True, sharey=True)
    img_count = 0
...
#                ax[r,c].imshow(all_imgs[img_count], interpolation='nearest', aspect='auto')
                ...
                border_color = '#40e843' if (all_type[img_count]=='TP') or (all_type[img_count]=='TN') else '#ea1212'
...
show_results(rand_choice_tp, rand_choice_tn, rand_choice_fp, rand_choice_fn, n_tp=3, n_tn=3, n_fp=3, n_fn=3, n_cols=3)
...
for tmp, img in enumerate(apply_images_list):
    ...
    img = image.load_img(os.path.join(img_path_apply, img), target_size=(x_pixels, y_pixels))
...
def show_rand_apply(index=None):
    ...
    if index==None:
        ...
        raise ValueError("There are only " + str(len(pred_prob_rev_apply)) + "images to display")
    elif index<0:
        raise ValueError("Index must be greater than or equal to zero")
    else:
        check_image_id = index
